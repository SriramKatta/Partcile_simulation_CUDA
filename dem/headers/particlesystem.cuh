...
struct Particlesystem
{
  ...
  void movetoGPU()
  {
    ...
    radiusptr = thrust::raw_pointer_cast(mdRadius.data());
  }

  void initDS(){
    double size = xyzmax - xyzmin;
    cellwidth = std::ceil(thrust::reduce(mdRadius.begin(), mdRadius.end(), 0, thrust::maximum<double>()));
  ...
  std::tuple<vec3d *, vec3d *, vec3d *, double *, double *>
  getDeviceParticlePoints()
  {
    ...
    return {posptr, velptr, accptr, massptr, radiusptr};
  }
